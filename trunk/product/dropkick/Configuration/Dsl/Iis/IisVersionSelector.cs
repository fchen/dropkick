...
namespace dropkick.Configuration.Dsl.Iis
{
    ...
    public static class IisVersionSelector
    {
        public static IisSiteOptions SelectTheCorrectConfig(Server server, string websiteName)
        {
            ...
            server.RegisterTask(new NoteTask("IIS Version Detection Used"));

            if (Environment.OSVersion.Version.Major == 1)
            {
                server.RegisterTask(new NoteTask("IIS Version was automatically set to IIS6"));
                return new Iis6TaskCfg(server, websiteName);
            }
            else if (Environment.OSVersion.Version.Major == 6)
            {
                server.RegisterTask(new NoteTask("IIS Version was automatically set to IIS7"));
